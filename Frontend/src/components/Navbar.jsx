    ...
    <header className="bg-primary text-white sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <form onSubmit={handleSearch} className="hidden md:flex items-center bg-white rounded-full px-3 py-1 flex-1 max-w-sm">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("nav.search")} className="ml-2 outline-none text-gray-800 w-full text-sm" />
        </form>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/cart" className="relative">
            <ShoppingCart />
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-accent text-xs rounded-full w-5 h-5 flex items-center justify-center">{cartCount}</span>
            )}
          </Link>

          {user ? (
            <div className="relative group" onClick={() => setOpen(!open)}>
              <button className="flex items-center gap-1"><User size={20} /> {user.name.split(" ")[0]}</button>
              {open && (
            ...
            <Link to="/login" className="bg-accent px-4 py-1.5 rounded-full text-sm font-semibold">{t("nav.login")}</Link>
          )}
